...
async function getSource(url) {
  ...
  const date = new Date();
  const data = await fetch('https://auto-references-api.deno.dev/api?url=' + encodeURIComponent(url)).then((res) => res.json());
...
let json;
try {
  json = parse(await Deno.readTextFile(data_dir() + '/Microsoft/Bibliography/Sources.xml'));
